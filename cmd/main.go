...
func init() {
	...
	utilruntime.Must(istioclientgov1alpha3.AddToScheme(scheme))
...
func main() {
	...
	flag.BoolVar(&isDeployment, "deployment", false, "Whether the application is running in deployment mode.")
	...
	// 	setupLog.Info("Try to start Accesserator with non-local Kubecontext. Exiting to prevent havoc.")
	// 	os.Exit(1)
	// }

	setupLog.Info(fmt.Sprintf("Starting Accesserator using Kube API server at %s", kubeconfig.Host))
	...
	mgr, err := ctrl.NewManager(kubeconfig, ctrl.Options{
		...
		LeaderElectionID:       "ea93bf51.kartverket.no",
