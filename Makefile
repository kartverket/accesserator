...
.PHONY: run-local
run-local: ensurelocal ensureaccesseratornotdeployed generate install webhooks ## Run Accesserator from your host.
...
apidocs:
	...
	cp config/crd/bases/accesserator.kartverket.no_*.yaml $(CRDOC_RESOURCES_DIR)/ || exit 1; \
...
@[ -f "$(1)-$(3)" ] && [ "$$(realpath "$(1)" 2>/dev/null || true)" = "$$(realpath "$(1)-$(3)" 2>/dev/null || true)" ] || { \
set -e; \
package=$(2)@$(3) ;\
echo "Downloading $${package}" ;\
rm -f "$(1)" ;\
GOBIN="$(LOCALBIN)" go install $${package} ;\
if [ "$$(realpath "$(1)" 2>/dev/null || true)" != "$$(realpath "$(1)-$(3)" 2>/dev/null || true)" ]; then \
	mv -f "$(1)" "$(1)-$(3)" ;\
fi ;\
} ;\
