...
var _ = Describe("DetermineReconcileAction", func() {
	...
	// ConfigMap is the simplest such type in this repo.
	...
	Context("when isDesiredNil is true", func() {
		It("returns RequiresDeleteAction when the resource exists and is owned by the SecurityConfig", func() {
		...
		It("returns RequiresNoAction when the resource exists but is NOT owned by the SecurityConfig", func() {
			action, err := reconciliation.DetermineReconcileAction[*corev1.ConfigMap](
				makeConfigMap("foo", "ns"),
				nil,
				true,
	...
	Context("when isDesiredNil is false", func() {
		...
		It("returns an error when the resource exists but is not owned by the SecurityConfig", func() {
			action, err := reconciliation.DetermineReconcileAction[*corev1.ConfigMap](
				makeConfigMap("foo", "ns"),
				&desired,
				false,
			...
			Expect(err.Error()).To(Equal("cannot update ns/foo as it is not owned by SecurityConfig"))
		...
		It("passes current and *desired to shouldUpdate verbatim", func() {
			...
			_, err := reconciliation.DetermineReconcileAction[*corev1.ConfigMap](
				...
				shouldUpdateSpy,
				true, // currentExists
				true, // currentIsOwnedBySecurityConfig
			)
			Expect(err).NotTo(HaveOccurred())
