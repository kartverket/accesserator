...
func GetBundleVerifier(sigstoreBundle *sigstorebundle.Bundle) (*verify.Verifier, error) {
	...
	if isSignedByPublicSigtoreInstance {
		...
		trustedRoot, getTrustedRootErr = PublicTrustedRoot()
...
func PublicTrustedRoot() (root.TrustedMaterial, error) {
	var err error
	publicGoodTufOnce.Do(func() {
		client, newTufErr := tuf.New(tuf.DefaultOptions().WithCachePath(config.Get().SigstoreTufCachePath))
		if newTufErr != nil {
			err = newTufErr
			return
		}
...
func GitHubTrustedRoot() (root.TrustedMaterial, error) {
	...
	githubTufOnce.Do(func() {
		client, newTufErr := tuf.New(tuf.DefaultOptions().
			...
			WithCachePath(config.Get().SigstoreTufCachePath))
		if newTufErr != nil {
			err = newTufErr
			return
		}
