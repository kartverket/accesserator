...
var _ = Describe("ID-porten ServiceEntry GetDesired", func() {
	objectMeta := metav1.ObjectMeta{Name: "security-config-idporten", Namespace: "team-a"}

	It("returns nil when ID-porten is disabled", func() {
		loadConfig(false)

		Expect(idportenserviceentry.GetDesired(objectMeta, state.IdPortenConfig{Enabled: false})).To(BeNil())
	})

	It("targets the test host when not running in production", func() {
		loadConfig(false)

		serviceEntry := idportenserviceentry.GetDesired(objectMeta, state.IdPortenConfig{Enabled: true})

		Expect(serviceEntry).NotTo(BeNil())
		Expect(serviceEntry.Spec.Hosts).To(ConsistOf(utilities.IdPortenTestHost))
	...
	It("targets the production host when running in production", func() {
		...
		Expect(serviceEntry).NotTo(BeNil())
		Expect(serviceEntry.Spec.Hosts).To(ConsistOf(utilities.IdPortenProdHost))
	})
})
