package ansattportenserviceentry
...
func GetDesired(objectMeta v1.ObjectMeta, ansattportenConfig state.AnsattportenConfig) *istionetworkingv1.ServiceEntry {
	if !ansattportenConfig.Enabled {
		return nil
	}

	ansattportenHost := utilities.AnsattportenTestHost
	if *config.Get().RunsInProduction {
		ansattportenHost = utilities.AnsattportenProdHost
	}
