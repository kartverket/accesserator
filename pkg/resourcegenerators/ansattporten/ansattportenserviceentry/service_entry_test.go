package ansattportenserviceentry_test
...
var _ = Describe("Ansattporten ServiceEntry GetDesired", func() {
	objectMeta := metav1.ObjectMeta{Name: "security-config-ansattporten", Namespace: "team-a"}

	It("returns nil when Ansattporten is disabled", func() {
		loadConfig(false)

		Expect(ansattportenserviceentry.GetDesired(objectMeta, state.AnsattportenConfig{Enabled: false})).To(BeNil())
	})

	It("targets the test host when not running in production", func() {
		loadConfig(false)

		serviceEntry := ansattportenserviceentry.GetDesired(objectMeta, state.AnsattportenConfig{Enabled: true})

		Expect(serviceEntry).NotTo(BeNil())
		Expect(serviceEntry.Spec.Hosts).To(ConsistOf(utilities.AnsattportenTestHost))
	...
	It("targets the production host when running in production", func() {
		...
		Expect(serviceEntry).NotTo(BeNil())
		Expect(serviceEntry.Spec.Hosts).To(ConsistOf(utilities.AnsattportenProdHost))
	})
})
