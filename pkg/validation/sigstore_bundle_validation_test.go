...
var _ = Describe("BuildGitHubSANRegex", func() {
	...
	Context("with a single org", func() {
		...
		BeforeEach(func() {
			var err error
			re, err = validation.BuildGitHubSANRegex([]string{"kartverket"})
	...
	Context("with multiple orgs", func() {
		...
		BeforeEach(func() {
			var err error
			re, err = validation.BuildGitHubSANRegex([]string{"kartverket", "accesserator"})
			Expect(err).NotTo(HaveOccurred())
		})
	...
	It("escapes regex metacharacters in org names so they are matched literally", func() {
		...
		re, err := validation.BuildGitHubSANRegex([]string{"a.b"})
		Expect(err).NotTo(HaveOccurred())
	...
	It("returns a regex that compiles via Go's regexp engine", func() {
		...
		Expect(re.String()).NotTo(BeEmpty())
	})
})
