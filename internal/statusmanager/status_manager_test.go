...
var _ = Describe("DetermineReconciliationState", func() {
					...
					Spec: v1alpha.SecurityConfigSpec{
						...
						Maskinporten: &v1alpha.MaskinportenSpec{
							...
							ClientRef: &v1alpha.ResourceRef{
								Name: v1alpha.ResourceName(utilities.NewMaskinportenNamer(v1alpha.SecurityConfig{Spec: v1alpha.SecurityConfigSpec{ApplicationRef: "my-app"}}).MaskinportenClientName()),
							},
						},
					},
				},
	...
	It("returns StateReady when Maskinporten is enabled through secretRef", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(*result).To(Equal(statusmanager.StateReady))
		Expect(scope.SecurityConfig.Status.MaskinportenSecretName).To(Equal(utilities.NewMaskinportenNamer(scope.SecurityConfig).SecretFromRefName()),
...
var _ = Describe("UpdateSecurityConfigStatus", func() {
	...
	Context("when OPA is enabled", func() {
		It("sets OpaBundleSource from bundle data in sorted order", func() {
			...
			Expect(updated.Status.OpaBundleSource.ConfigMapName).To(Equal(utilities.NewOpaNamer(scope.SecurityConfig).ConfigMapName()))
