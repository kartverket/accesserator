...
var _ = Describe("HandleAzureAdApplicationEvent", func() {
	It("enqueues SecurityConfigs in the same namespace referencing the AzureAdApplication", func() {
		clientObj := &naisiov1.AzureAdApplication{ObjectMeta: metav1.ObjectMeta{Name: "az-client", Namespace: "team-a"}}
		...
		scMatch1 := &v1alpha.SecurityConfig{
			...
			Spec: v1alpha.SecurityConfigSpec{
				...
				EntraID: &v1alpha.EntraIDSpec{
					ClientRef: &v1alpha.ResourceRef{Name: "az-client"},
				},
			},
		}
		...
		h := eventhandler.HandleAzureAdApplicationEvent(c)
