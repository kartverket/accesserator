...
echo "🤞  Creating namespace: tokenx-api"
...
set +e
output=$("${KUBECTL_BIN}" create namespace "tokenx-api" --context "$KUBECONTEXT" 2>&1)
exit_code=$?
set -e

# Check the exit code and output
if [ $exit_code -eq 0 ]; then
    echo "✅  Namespace 'tokenx-api' created successfully"
elif echo "$output" | grep -qiE "already exists|AlreadyExists"; then
    echo "✅  Namespace 'tokenx-api' already exists, continuing..."
else
    echo -e "❌  Error creating 'tokenx-api' namespace:"
    echo "$output"
    exit 1
fi
...
spec:
  ...
  env:
    ...
    - name: AUTH_ACCEPTED_AUDIENCE
      value: http://tokendings.tokenx-api:7456/registration/client
    - name: AUTH_CLIENT_ID
